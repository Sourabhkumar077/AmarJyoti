...
export const getProductsHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (subcategory) {
      query.subcategory = {
        $regex: new RegExp(String(subcategory).trim(), "i"),
      };
    }
...
export const updateProductHandler = async (req: Request, res: Response) => {
  try {
    ...
    if (newDiscount > 0) {
      updatePayload.salePrice = Math.round(
...
// 6. REPAIR DATA TOOL
export const repairProductPricesHandler = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `Successfully repaired prices for ${count} products.` });
  } catch (error: any) {
